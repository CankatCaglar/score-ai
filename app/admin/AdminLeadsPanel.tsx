...
import type { AdminListSort } from "@/actions/admin";
...
export type AdminLeadRow = {
  ...
  /** Optional secondary line under email (name, company, etc.) */
  subtitle?: string | null;
  /** Optional table column (e.g. analysis count, plan) */
  meta?: string | null;
};
...
type AdminLeadsPanelProps = {
  ...
  list: (sort: AdminListSort) => Promise<AdminLeadRow[]>;
...
export function AdminLeadsPanel({
  ...
  const [entries, setEntries] = useState<AdminLeadRow[]>([]);
  ...
  const showMeta = Boolean(metaColumnLabel);

  const load = useCallback(
    async (nextSort: AdminListSort) => {
      try {
        const data = await list(nextSort);
        setEntries(data);
      } catch (error) {
  ...
  const handleDelete = async (entry: AdminLeadRow) => {
    const confirmed = window.confirm(deleteConfirmLabel(entry.email));
    if (!confirmed) return;
  ...
  const handleExportCsv = () => {
    ...
    const headers = [
      ...
      "E-posta",
      "Dil",
      ...(showMeta && metaColumnLabel ? [metaColumnLabel] : []),
      ...(exportRows.some((r) => r.subtitle) ? ["Detay"] : []),
      "ID",
      "Eklenme",
    ];
    ...
    const lines = [
      ...
      ...exportRows.map((row) => {
        const cols: Array<string | number> = [row.sira, row.email, row.dil];
        if (showMeta) cols.push(row.meta);
        if (exportRows.some((r) => r.subtitle)) cols.push(row.subtitle);
        cols.push(row.id, row.eklenme);
    ...
    downloadBlob(blob, `${exportSlug}-${sanitizeFilePart(now)}.csv`);
  };
...
<body>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th>E-posta</th>
        <th>Dil</th>
        ${showMeta && metaColumnLabel ? `<th>${metaColumnLabel}</th>` : ""}
        <th>ID</th>
        <th>Eklenme</th>
      </tr>
    </thead>
  ...
  const handleExportWord = () => {
    ...
    const now = new Date().toISOString().slice(0, 10);
    downloadBlob(blob, `${exportSlug}-${sanitizeFilePart(now)}.doc`);
  };
  ...
  const handleExportPdf = () => {
    ...
    autoTable(doc, {
      ...
      body: exportRows.map((row) => {
        const cols: Array<string | number> = [row.sira, row.email, row.dil];
        if (showMeta) cols.push(row.meta);
        cols.push(row.id, row.eklenme.replaceAll("ı", "i"));
        return cols;
      }),
      theme: "striped",
      styles: {
    ...
    doc.save(`${exportSlug}-${sanitizeFilePart(now)}.pdf`);
  };

  const colSpan = showMeta ? 7 : 6;
    ...
    <>
      ...
      <section className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <article className="rounded-2xl border border-brand-dark/10 bg-bg-light p-4 shadow-sm">
          ...
          <p className="text-xs font-medium text-brand-dark/80">{totalLabel}</p>
          <p className="mt-1 text-3xl font-semibold text-brand-dark">
            {entries.length.toLocaleString("tr-TR")}
          </p>
          <p className="mt-1 text-xs text-brand-dark/45">Toplam kayıt</p>
        </article>
        ...
        {errorCode === "FIREBASE_ADMIN_NOT_CONFIGURED" ? (
          <div className="px-6 py-16 text-center">
            ...
            <p className="mx-auto mt-2 max-w-md text-sm text-brand-dark/60">
              Kayıtları görmek için <code>.env.local</code> içinde{" "}
                    ...
                    filtered.map((entry, index) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-medium text-brand-dark">
                          ...
                          {entry.subtitle ? (
                            <p className="mt-0.5 text-xs font-normal text-brand-dark/45">
                              {entry.subtitle}
                            </p>
                          ) : null}
                        </td>
