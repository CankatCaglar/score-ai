...
export async function runAnalysisJobSubmission(input: {
  ownerEmail: string;
  formData: FormData;
  guestId?: string;
  ...
  if (!hasFile && !hasUrl) {
    return { ok: false, status: 400, error: "INPUT_REQUIRED" };
  }

  if (hasFile && file instanceof File && file.size > 20 * 1024 * 1024) {
    return {
      ok: false,
      status: 422,
